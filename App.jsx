...
export default function App() {
  ...
  useEffect(() => {
    const startApp = async () => {
      try {
        ...
        setInitError("Firebase接続エラー。匿名認証が有効か確認してください。");
      }
    };
    startApp();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const resCol = collection(fb.db, 'artifacts', appIdString, 'public', 'data', 'reservations');
      const custDoc = doc(fb.db, 'artifacts', appIdString, 'public', 'data', 'customers', customerName);
      await addDoc(resCol, {
        customerName,
  ...
  const adjustTickets = async (custId, amount) => {
    if (!user || !fb.db) return;
    const custDoc = doc(fb.db, 'artifacts', appIdString, 'public', 'data', 'customers', custId);
  ...
  if (initError) return (
    <div className="min-h-screen bg-red-50 flex items-center justify-center p-6 text-center">
      <div className="bg-white p-10 rounded-[48px] shadow-2xl border-4 border-red-100 max-w-md font-black">
        ...
        <p className="text-slate-600 mb-8 font-bold">{String(initError)}</p>
        <button onClick={() => window.location.reload()} className="w-full bg-slate-800 text-white py-5 rounded-full">再読み込み</button>
      </div>
    </div>
  );
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#fcfaf8] flex items-center justify-center font-black text-[#b4927b] flex-col space-y-8 p-6 text-center">
      ...
      <span className="text-3xl tracking-widest">接続を確立しています...</span>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#fcfaf8] text-slate-900 font-sans pb-32 select-none overflow-x-hidden">
      <div className="bg-white border-b-2 border-gray-100 px-6 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm text-center">
        ...
        {isAdminMode ? (
          <div className="flex items-center space-x-3 text-center"><span className="text-[12px] font-black text-[#b4927b] bg-[#fcf8f5] px-3 py-1.5 rounded-full border-2 flex items-center shadow-xs text-center"><Unlock size={14} className="mr-1"/> 管理中</span><button onClick={() => { setIsAdminMode(false); setStep(1); }} className="text-[12px] font-black text-slate-500 text-center">終了</button></div>
        ) : (
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6 text-center">
        {step === 6 && (
          <div className="animate-in zoom-in duration-300 max-w-sm mx-auto pt-16 px-4 text-center">
            <Lock size={64} className="mx-auto mb-8 text-[#b4927b] text-center" />
            <h2 className="text-3xl font-black mb-6 text-center">管理者認証</h2>
            <form onSubmit={handleAdminLogin} className="space-y-6 text-center">
              <input type="password" autoFocus value={passInput} onChange={(e) => setPassInput(e.target.value)} placeholder="合言葉" className="w-full bg-white border-2 border-gray-200 rounded-[32px] py-6 text-center text-3xl font-black tracking-[0.5em] focus:outline-none focus:border-[#b4927b] shadow-inner text-center" />
        ...
        {step === 3 && adminTab === 'ledger' && (
          <div className="animate-in fade-in pt-8 text-center font-black text-center">
            ...
            <div className="bg-white rounded-[56px] shadow-2xl border-4 border-white overflow-x-auto max-h-[70vh] no-scrollbar text-center">
              <table className="w-full border-collapse table-fixed min-w-[500px] text-center">
                ...
                <tbody className="divide-y-2 divide-slate-50 text-center font-black">
                  {TIME_SLOTS.map((time) => (
                    <tr key={time} className="h-20 hover:bg-gray-50 text-center font-black text-center">
                      ...
                      {Array.from({ length: 7 }, (_, i) => {
                        ...
                        return <td key={i} className="p-0 border-r last:border-r-0 text-center font-black">
                          {isAdminMode ? (
                            ...
                            <button disabled={status.disabled} onClick={() => { setSelectedDate(d); setTargetTime(status.groupInfo ? status.groupInfo.time : time); setStep(4); }} className={`w-full h-full flex flex-col items-center justify-center transition-all text-center ${status.disabled ? 'text-gray-200 bg-slate-50 text-center' : 'text-[#b4927b] hover:bg-[#fcf8f5] active:scale-90 font-black text-center'}`}>
            ...
            <div className="mt-12 flex justify-center space-x-12 text-lg font-black uppercase bg-white py-8 rounded-[40px] shadow-sm border-2 text-center font-black">
              ...
              <span className="flex items-center text-center"><span className="text-orange-500 mr-3 text-4xl text-center text-center">△</span> 混雑</span>
              <span className="flex items-center text-center"><span className="text-gray-300 mr-3 text-4xl text-center text-center">×</span> 満席</span>
            </div>
          </div>
        )}
          ...
          <div className="animate-in slide-in-from-right pt-10 text-left text-left text-left text-left">
            ...
            <div className="bg-white rounded-[72px] p-16 border-2 shadow-2xl mb-16 text-left font-black text-left text-left">
              <div className="space-y-12 mb-12 text-left text-left">
                <div className="flex justify-between border-b-2 pb-8 text-slate-500 text-xl text-left text-left"><span className="text-left font-black text-left">メニュー</span><span className="text-3xl text-[#b4927b] font-black text-right text-right">{selectedMenu ? selectedMenu.name : '少人数制グループ'}</span></div>
                <div className="flex flex-col border-b-2 pb-8 text-slate-500 text-xl text-left text-left"><span className="mb-4 text-left font-black text-left">日時</span><span className="text-3xl text-slate-900 leading-relaxed font-black text-left text-left">{selectedDate.toLocaleDateString('ja-JP', { month: 'long', day: 'numeric' })} ({selectedDate.toLocaleDateString('ja-JP', { weekday: 'short' })}) <br className="text-left"/> {targetTime} 〜 {calculateEndTime(selectedDate, targetTime, selectedMenu ? selectedMenu.duration : GROUP_MIN)}</span></div>
              </div>
              <div className="bg-[#fcf8f5] rounded-[48px] p-12 shadow-inner border-2 border-white text-center text-center text-center">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/98 border-t-4 border-gray-50 px-10 py-10 flex justify-around items-center z-30 shadow-[0_-20px_60px_rgba(0,0,0,0.08)] backdrop-blur-2xl text-center text-center">
        ...
        <div className={`flex flex-col items-center cursor-pointer transition-all text-center text-center ${isAdminMode ? 'text-[#b4927b] scale-125' : 'text-slate-300'}`} onClick={() => { if(!isAdminMode) setStep(6); else setAdminTab('ledger'); }}>
          <CalendarCheck size={56} strokeWidth={2.5} className="text-center text-center"/><span className="text-[16px] font-black mt-3 uppercase tracking-widest text-center text-center">台帳</span>
        </div>
        <div className="flex flex-col items-center text-gray-300 text-center text-center"><Info size={56} strokeWidth={2.5} className="text-center text-center"/><span className="text-[16px] font-black mt-3 uppercase tracking-widest text-center text-center text-center">情報</span></div>
      </footer>
    </div>
  );
}
