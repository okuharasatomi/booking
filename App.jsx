...
const appId = 'lesson-with-satomio'; // Firestoreのパスに使用する固定ID
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        setErrorMsg(`認証エラー: Firebaseコンソールで匿名ログインを有効にしてください。`);
      }
    };
    initAuth();
  ...
  const handleAdminSlotClick = async (date, time, bookings) => {
    ...
    if (bookings.length > 0) {
      ...
      if (window.confirm(`${time} をお休みにしますか？`)) {
  ...
  if (errorMsg) return <div className="min-h-screen bg-red-50 flex items-center justify-center p-6 text-center font-bold text-red-600">{errorMsg}</div>;
  if (!user) return <div className="min-h-screen bg-[#fcfaf8] flex items-center justify-center font-black text-[#b4927b] text-xl">接続中...</div>;
      ...
      <main className="max-w-4xl mx-auto p-4">
        {step === 6 && (
          <div className="animate-in zoom-in duration-300 max-w-sm mx-auto pt-16 px-4 text-center">
            ...
            <form onSubmit={handleAdminLogin} className="space-y-6">
              ...
              <button className="w-full bg-[#b4927b] text-white font-black py-6 rounded-[32px] shadow-xl text-xl">ログイン</button>
        ...
        {!isAdminMode && step === 1 && (
          <div className="animate-in fade-in pt-10 text-left">
            ...
            <div className="grid grid-cols-1 gap-6">
              <button onClick={() => {setLessonCategory('private'); setStep(2);}} className="w-full bg-white p-8 rounded-[48px] border-2 shadow-md flex items-center justify-between active:bg-gray-50">
                ...
                <User size={48} className="text-[#b4927b]"/>
              </button>
              <button onClick={() => {setLessonCategory('group'); setSelectedMenu(null); setStep(3);}} className="w-full bg-white p-8 rounded-[48px] border-2 shadow-md flex items-center justify-between active:bg-gray-50">
                ...
                <Users size={48} className="text-[#b4927b]"/>
              </button>
            </div>
          </div>
        )}
          ...
          <div className="animate-in slide-in-from-right pt-10 text-left">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {PRIVATE_MENUS.map(menu => (
                <button key={menu.id} onClick={() => {setSelectedMenu(menu); setStep(3);}} className="bg-white p-10 rounded-[40px] border-2 text-center shadow-md active:scale-95 font-black text-2xl">
                  {menu.name.replace('個人 ', '')}<div className="text-base text-slate-500 mt-2">{menu.description}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 3 && adminTab === 'ledger' && (
          <div className="animate-in fade-in pt-8 text-center">
            {!isAdminMode && <button onClick={() => setStep(lessonCategory === 'private' ? 2 : 1)} className="text-base font-black text-slate-500 mb-6 flex items-center"><ChevronLeft size={24}/> 選び直す</button>}
            {isAdminMode && (
            ...
            <div className="bg-white rounded-[48px] shadow-2xl border-2 overflow-x-auto max-h-[70vh] no-scrollbar">
              <table className="w-full border-collapse table-fixed min-w-[400px]">
                ...
                <tbody className="divide-y text-center">
                  {TIME_SLOTS.map((time) => (
                    <tr key={time} className="h-16 hover:bg-gray-50 text-center font-black">
          ...
          <div className="animate-in fade-in pt-10 px-2 space-y-6">
            {customers.map(cust => (
              <div key={cust.id} className="bg-white p-8 rounded-[56px] border-2 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 font-black">
                ...
                <div className="flex space-x-3"><button onClick={() => adjustTickets(cust.id, -1)} className="p-5 bg-slate-100 rounded-3xl"><Minus size={32}/></button><button onClick={() => adjustTickets(cust.id, 1)} className="p-5 bg-[#fcf8f5] rounded-3xl"><Plus size={32}/></button><button onClick={() => adjustTickets(cust.id, 4)} className="px-8 py-5 bg-[#b4927b] text-white rounded-[24px] text-lg shadow-2xl active:scale-95 transition-all">＋4枚</button></div></div>
              </div>
            ))}
          </div>
        )}
        ...
        {!isAdminMode && step === 4 && (
          <div className="animate-in slide-in-from-right pt-10 px-2 text-left">
            <h2 className="text-xl font-black mb-8 border-l-4 border-[#b4927b] pl-5 text-slate-900">ご予約内容の確認</h2>
            <div className="bg-white rounded-[64px] p-12 border-2 shadow-2xl mb-12 text-left font-black">
              ...
              <div className="bg-[#fcf8f5] rounded-[40px] p-10 shadow-inner">
                ...
                <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-white border-2 rounded-3xl py-6 px-8 text-2xl focus:border-[#b4927b] outline-none transition-all shadow-sm" placeholder="お名前を入力" />
              </div>
            </div>
        ...
        {!isAdminMode && step === 5 && (
          <div className="text-center py-32 animate-in zoom-in px-4">
            <Check size={80} strokeWidth={4} className="mx-auto mb-12 text-green-500 bg-green-50 rounded-full p-4 shadow-xl" />
