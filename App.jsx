...
export default function App() {
  ...
  const [initError, setInitError] = useState(!hasValidConfig ? "Firebaseの設定が読み込めませんでした。" : null);
  ...
  if (initError) {
    return (
      <div className="min-h-screen bg-red-50 flex items-center justify-center p-4 text-center">
        <div className="bg-white p-8 rounded-[40px] shadow-2xl border-4 border-red-100 max-w-sm">
          <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-6" />
          <h2 className="text-2xl font-black text-slate-800 mb-4 tracking-tight">接続エラー</h2>
          <p className="text-slate-600 font-bold mb-8 leading-relaxed">{initError}</p>
          <button onClick={() => window.location.reload()} className="w-full bg-slate-800 text-white font-black py-4 rounded-3xl active:scale-95 transition-all">再読み込みする</button>
        </div>
      </div>
    );
  }
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#fcfaf8] flex items-center justify-center font-black text-[#b4927b] flex-col space-y-4">
      ...
      <span className="text-xl tracking-widest">接続を確立しています...</span>
      <p className="text-xs text-slate-400 font-bold">※エラーが続く場合はFirebaseの匿名認証を有効にしてください</p>
    </div>
  );
      ...
      <main className="max-w-4xl mx-auto p-2 sm:p-4">
          ...
          <div className="animate-in fade-in pt-8 px-2 text-left">
            ...
            <div className="grid grid-cols-1 gap-5">
              <button onClick={() => {setLessonCategory('private'); setStep(2);}} className="w-full bg-white p-7 rounded-[40px] border-2 border-gray-100 shadow-md flex items-center justify-between hover:border-[#b4927b] group transition-all">
                <div className="text-left"><div className="text-xl font-black text-slate-900 text-left">個人レッスン</div><div className="text-sm text-slate-600 mt-1 font-bold text-left">1枠25分〜。マンツーマン</div></div>
                <div className="bg-[#fcf8f5] p-5 rounded-3xl text-[#b4927b] group-hover:bg-[#b4927b] group-hover:text-white transition-colors shadow-sm"><User size={40}/></div>
              </button>
              <button onClick={() => {setLessonCategory('group'); setSelectedMenu(null); setStep(3);}} className="w-full bg-white p-7 rounded-[40px] border-2 border-gray-100 shadow-md flex items-center justify-between hover:border-[#b4927b] group transition-all">
                <div className="text-left"><div className="text-xl font-black text-slate-900 text-left">少人数制グループ</div><div className="text-sm text-slate-600 mt-1 font-bold text-left">50分。定員3名。水・木・金</div></div>
                <div className="bg-[#fcf8f5] p-5 rounded-3xl text-[#b4927b] group-hover:bg-[#b4927b] group-hover:text-white transition-colors shadow-sm"><Users size={40}/></div>
              </button>
            </div>
          </div>
        )}
        ...
        {!isAdminMode && step === 2 && (
          <div className="animate-in slide-in-from-right pt-8 px-2 text-left">
            ...
            <h2 className="text-base font-black mb-6 border-l-4 border-[#b4927b] pl-3 text-slate-900 text-left">レッスン数を選択</h2>
        ...
        {step === 3 && adminTab === 'ledger' && (
          <div className="animate-in fade-in pt-6 text-center">
            ...
            <div className="bg-white rounded-[40px] shadow-2xl border-2 border-gray-100 overflow-x-auto max-h-[75vh] no-scrollbar">
              <table className="w-full border-collapse table-fixed min-w-[360px]">
                ...
                <tbody className="divide-y divide-gray-100 text-center">
                  {TIME_SLOTS.map((time) => (
                    <tr key={time} className="h-14 hover:bg-gray-50/50 transition-colors text-center text-center">
                          ...
                          <td key={i} className="p-0 text-center relative border-r border-gray-100 last:border-r-0 text-center">
                            {isAdminMode ? (
                              ...
                              <button disabled={status.disabled} onClick={() => { setSelectedDate(date); setTargetTime(status.groupInfo ? status.groupInfo.time : time); setStep(4); }} className={`w-full h-full flex flex-col items-center justify-center transition-all text-center ${status.disabled ? 'text-gray-200 cursor-not-allowed bg-slate-50/20' : 'text-[#b4927b] hover:bg-[#fcf8f5] active:scale-90 font-black'}`}>
                                {status.groupInfo && (<span className="text-[9px] text-[#b4927b] font-black leading-none mb-1 bg-white px-1.5 py-0.5 border-2 rounded-full shadow-xs whitespace-nowrap text-center text-center">{status.groupInfo.time}</span>)}
            ...
            <div className="mt-8 flex justify-center space-x-8 text-[12px] text-slate-900 font-black uppercase tracking-widest bg-white py-5 rounded-[24px] shadow-sm border-2 border-gray-100 text-center text-center">
              ...
              <span className="flex items-center text-center"><span className="text-orange-500 mr-2 text-2xl font-black text-center text-center text-center">△</span> 残りわずか</span>
              <span className="flex items-center text-center"><span className="text-gray-300 mr-2 text-2xl font-black text-center text-center text-center">×</span> 満席/休</span>
            </div>
          </div>
        )}
              ...
              <div className="space-y-5">
                {customers.map(cust => (
                  <div key={cust.id} className="bg-white p-6 rounded-[40px] border-2 border-gray-100 shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-5 transition-all text-left">
                    <div className="flex items-center space-x-5 text-left text-left text-left">
                      <div className="w-16 h-16 bg-[#fcf8f5] text-[#b4927b] rounded-3xl flex items-center justify-center font-black text-3xl shadow-inner border-2 border-white text-center text-center">{cust.name?.charAt(0)}</div>
                      <div className="text-left text-slate-900 text-left text-left text-left text-left"><div className="text-xl font-black text-left text-left">{cust.name} 様</div><div className="text-[12px] text-slate-500 font-bold uppercase tracking-tight mt-1 text-left text-left text-left">最新予約: {cust.lastReservedAt?.toDate().toLocaleDateString()}</div></div>
                    </div>
                    <div className="flex items-center justify-between sm:justify-end space-x-6 border-t sm:border-t-0 pt-5 sm:pt-0 text-left text-left text-left text-left">
                      <div className="text-center sm:text-right text-left text-left text-left text-left text-left"><div className="text-[11px] text-slate-400 font-black uppercase tracking-widest leading-none mb-2 text-left text-left text-left text-left">残チケット</div><div className={`text-3xl font-black text-left text-left text-left ${ (cust.tickets || 0) <= 0 ? 'text-red-500' : 'text-[#b4927b]'}`}>{cust.tickets || 0} <span className="text-lg text-left">枚</span></div></div>
                      <div className="flex space-x-2 text-center text-center text-center text-center">
                        <button onClick={() => adjustTickets(cust.id, -1)} className="p-4 bg-slate-100 text-slate-800 rounded-2xl active:scale-90 transition-all border-2 border-white shadow-sm text-center text-center text-center"><Minus size={24}/></button>
                        <button onClick={() => adjustTickets(cust.id, 1)} className="p-4 bg-[#fcf8f5] text-[#b4927b] rounded-2xl active:scale-90 transition-all border-2 border-white shadow-sm text-center text-center text-center"><Plus size={24}/></button>
                        <button onClick={() => adjustTickets(cust.id, 4)} className="px-6 py-4 bg-[#b4927b] text-white rounded-2xl text-sm font-black shadow-xl text-center text-center text-center">＋4枚</button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {!isAdminMode && step === 4 && (
          <div className="animate-in slide-in-from-right pt-8 px-2 text-center text-center text-center text-center text-center">
            <h2 className="text-lg font-black mb-6 border-l-4 border-[#b4927b] pl-4 text-left text-slate-900 text-left text-left">ご予約内容の確認</h2>
            <div className="bg-white rounded-[56px] p-10 border-2 border-gray-100 shadow-2xl mb-10 text-left text-left text-left text-left text-left">
              <div className="space-y-8 mb-10 text-left text-left text-left text-left text-left text-left">
                <div className="flex justify-between items-center border-b-2 border-gray-50 pb-5 text-left text-left text-left text-left"><span className="text-[13px] font-black text-slate-400 uppercase tracking-widest text-left text-left text-left">メニュー</span><span className="text-lg font-black text-[#b4927b] text-left text-left text-left">{selectedMenu ? selectedMenu.name : '少人数制グループ'}</span></div>
                <div className="flex flex-col border-b-2 border-gray-50 pb-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><span className="text-[13px] font-black text-slate-400 uppercase tracking-widest mb-2 text-left text-left text-left text-left">予約日時</span><span className="text-xl font-black text-slate-900 leading-relaxed text-left text-left text-left text-left text-left text-left text-left">{selectedDate.toLocaleDateString('ja-JP', { month: 'long', day: 'numeric' })} ({selectedDate.toLocaleDateString('ja-JP', { weekday: 'short' })}) <br className="sm:hidden text-left text-left text-left text-left" /> {targetTime} 〜 {calculateEndTime(selectedDate, targetTime, selectedMenu ? selectedMenu.duration : GROUP_MIN)}</span></div>
              </div>
              <div className="bg-[#fcf8f5] rounded-[32px] p-8 border-2 border-[#f5ece5] shadow-inner text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                <label className="text-[12px] font-black text-[#b4927b] mb-3 block uppercase tracking-[0.2em] text-left text-left text-left text-left">お名前（LINE名）</label>
                <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-white border-2 border-[#f5ece5] rounded-2xl py-5 px-6 text-xl font-black text-slate-900 focus:border-[#b4927b] outline-none transition-all shadow-sm text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left" placeholder="お名前を入力" />
              </div>
            </div>
            <button onClick={handleSubmit} disabled={loading || !customerName.trim()} className="w-full bg-[#b4927b] text-white font-black py-7 rounded-full shadow-2xl active:scale-95 disabled:bg-gray-200 text-xl transition-all shadow-[#b4927b]/30 text-center text-center text-center text-center text-center text-center text-center">{loading ? "送信中..." : "上記の内容で予約確定"}</button>
          </div>
        )}

        {!isAdminMode && step === 5 && (
          <div className="text-center py-24 animate-in zoom-in px-4 text-center text-center text-center text-center text-center">
            <div className="w-32 h-32 bg-green-50 text-green-600 rounded-full flex items-center justify-center mx-auto mb-10 shadow-xl border-4 border-white text-center text-center text-center text-center text-center text-center text-center text-center text-center"><Check size={64} strokeWidth={4} /></div>
            <h2 className="text-4xl font-serif font-black mb-5 text-slate-900 text-center text-center text-center text-center text-center text-center text-center text-center">予約完了！</h2>
            <p className="text-lg text-slate-600 font-bold mb-16 leading-relaxed text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">ご予約ありがとうございます。<br/>当日お会いできるのを楽しみにしております。</p>
            <button onClick={() => setStep(1)} className="w-full py-6 bg-white border-2 border-gray-100 text-[#b4927b] font-black rounded-full shadow-xl text-lg hover:bg-gray-50 transition-all text-center text-center text-center text-center text-center text-center text-center">トップへ戻る</button>
          </div>
        )}
      </main>
      
      {/* Footer Nav */}
      <footer className="fixed bottom-0 left-0 right-0 bg-white/98 border-t-2 border-gray-100 px-8 py-6 flex justify-around items-center z-30 shadow-[0_-12px_40px_rgba(0,0,0,0.04)] backdrop-blur-lg text-center text-center text-center text-center text-center">
        <div className={`flex flex-col items-center cursor-pointer transition-all text-center text-center text-center text-center ${!isAdminMode ? 'text-[#b4927b] scale-110' : 'text-slate-300'}`} onClick={() => {setStep(1); setIsAdminMode(false);}}>
          <CalendarIcon size={32} strokeWidth={2.5}/><span className="text-[12px] font-black mt-1.5 uppercase tracking-wider text-center text-center text-center text-center">予約</span>
        </div>
        <div className={`flex flex-col items-center cursor-pointer transition-all text-center text-center text-center text-center ${isAdminMode ? 'text-[#b4927b] scale-110' : 'text-slate-300'}`} onClick={() => { if(!isAdminMode) setStep(6); else setAdminTab('ledger'); }}>
          <CalendarCheck size={32} strokeWidth={2.5}/><span className="text-[12px] font-black mt-1.5 uppercase tracking-wider text-center text-center text-center text-center text-center text-center text-center text-center">台帳</span>
        </div>
        <div className="flex flex-col items-center text-gray-300 cursor-help text-center text-center text-center text-center text-center"><Info size={32} strokeWidth={2.5}/><span className="text-[12px] font-black mt-1.5 uppercase tracking-wider text-center text-center text-center text-center text-center">情報</span></div>
      </footer>
    </div>
  );
}
