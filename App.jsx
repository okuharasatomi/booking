...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setInitError("初期化に失敗しました。Firebaseの設定を確認してください。");
      }
    };
    init();
  }, []);
  ...
  useEffect(() => {
    ...
    }, (err) => console.error("Firestore Res Error:", err));
    ...
    const unsubCust = onSnapshot(query(custCol), (snap) => {
      ...
      setCustomers(data);
    }, (err) => console.error("Firestore Cust Error:", err));
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6">
        {step === 6 && (
          <div className="animate-in zoom-in duration-300 max-w-sm mx-auto pt-16 px-4 text-center">
            ...
            <form onSubmit={handleAdminLogin} className="space-y-6">
              ...
              <button type="button" onClick={() => setStep(1)} className="text-base text-slate-500 font-black mt-6 border-b-2 text-center">戻る</button>
            </form>
          </div>
        )}
        ...
        {!isAdminMode && step === 1 && (
          <div className="animate-in fade-in pt-10 text-left">
            <h2 className="text-2xl font-black mb-10 border-l-8 border-[#b4927b] pl-6 text-slate-900">メニュー選択</h2>
            <div className="grid grid-cols-1 gap-8 text-left">
              <button onClick={() => {setLessonCategory('private'); setStep(2);}} className="w-full bg-white p-10 rounded-[56px] border-2 shadow-lg flex items-center justify-between active:bg-gray-50 group transition-all text-left">
                <div className="text-left font-black"><div className="text-3xl text-slate-900 text-left">個人レッスン</div><div className="text-lg text-slate-600 mt-2 font-black text-left">1枠25分〜。マンツーマン</div></div>
                <User size={64} className="text-[#b4927b] group-hover:scale-110 transition-transform text-center"/>
              </button>
              <button onClick={() => {setLessonCategory('group'); setSelectedMenu(null); setStep(3);}} className="w-full bg-white p-10 rounded-[56px] border-2 shadow-lg flex items-center justify-between active:bg-gray-50 group transition-all text-left">
                <div className="text-left font-black"><div className="text-3xl text-slate-900 text-left">少人数制グループ</div><div className="text-lg text-slate-600 mt-2 font-black text-left">50分。定員3名。水・木・金</div></div>
                <Users size={64} className="text-[#b4927b] group-hover:scale-110 transition-transform text-center"/>
              </button>
            </div>
          </div>
        )}
        ...
        {step === 3 && adminTab === 'ledger' && (
          <div className="animate-in fade-in pt-8 text-center font-black text-center">
            {!isAdminMode && <button onClick={() => setStep(lessonCategory === 'private' ? 2 : 1)} className="text-xl font-black text-slate-500 mb-8 flex items-center hover:text-black text-center"><ChevronLeft size={32}/> 選び直す</button>}
            {isAdminMode && (
              <div className="flex bg-white rounded-[32px] p-2 border-2 mb-10 max-w-sm mx-auto shadow-sm text-center">
            ...
            <div className="bg-white rounded-[56px] shadow-2xl border-4 border-white overflow-x-auto max-h-[70vh] no-scrollbar text-center">
              <table className="w-full border-collapse table-fixed min-w-[500px] text-center">
                <thead className="sticky top-0 z-20 shadow-md bg-[#fcf8f5] text-center">
                  <tr className="text-center"><th className="py-8 w-24 text-xl font-black text-center">時間</th>
                    {Array.from({ length: 7 }, (_, i) => {
                      const d = new Date(startDate); d.setDate(d.getDate() + i);
                      return <th key={i} className="py-8 font-black text-center"><div className={`text-sm mb-1 text-center ${d.getDay() === 0 ? 'text-red-500' : d.getDay() === 6 ? 'text-blue-500' : 'text-slate-500'}`}>{['日','月','火','水','木','金','土'][d.getDay()]}</div><div className="text-2xl text-center">{d.getDate()}</div></th>;
                    })}
                  </tr>
                </thead>
                <tbody className="divide-y-2 divide-slate-50 text-center font-black">
                  {TIME_SLOTS.map((time) => (
                    <tr key={time} className="h-20 hover:bg-gray-50 text-center font-black">
                      <td className="sticky left-0 bg-white border-r-2 text-lg text-slate-900 text-center font-black">{time}</td>
                      {Array.from({ length: 7 }, (_, i) => {
                        const d = new Date(startDate); d.setDate(d.getDate() + i);
                        const status = getSlotStatus(d, time);
                        return <td key={i} className="p-0 border-r last:border-r-0 text-center font-black">
                          {isAdminMode ? (
                            <button onClick={() => handleAdminSlotClick(d, time, status.bookings)} className="w-full h-full flex flex-col items-center justify-center transition-all hover:bg-gray-100 text-center">
                              {status.bookings?.length > 0 ? <div className="text-[11px] bg-[#b4927b] text-white p-2 rounded-xl w-full truncate leading-tight text-center">{status.bookings[0].customerName}</div> : <span className="text-gray-100 text-2xl text-center">○</span>}
                            </button>
                          ) : (
                            <button disabled={status.disabled} onClick={() => { setSelectedDate(d); setTargetTime(status.groupInfo ? status.groupInfo.time : time); setStep(4); }} className={`w-full h-full flex flex-col items-center justify-center transition-all text-center ${status.disabled ? 'text-gray-200 bg-slate-50' : 'text-[#b4927b] hover:bg-[#fcf8f5] active:scale-90 font-black'}`}>
                              ...
                              <span className="text-5xl text-center font-black">{status.mark}</span>
                            </button>
                          )}
                        </td>;
                      })}
        ...
        {isAdminMode && adminTab === 'tickets' && (
          <div className="animate-in fade-in pt-10 space-y-8 text-left">
            {customers.map(cust => (
              <div key={cust.id} className="bg-white p-10 rounded-[64px] border-2 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-10 font-black text-left">
                <div className="flex items-center space-x-8 text-left font-black"><div className="w-24 h-24 bg-[#fcf8f5] text-[#b4927b] rounded-[40px] flex items-center justify-center text-5xl border-4 border-white shadow-inner text-center font-black">{cust.name?.charAt(0)}</div><div className="text-left text-3xl font-black">{cust.name} 様</div></div>
                <div className="flex items-center gap-12 text-left font-black"><div className="text-center font-black text-center"><div className="text-base text-slate-400 mb-2 uppercase text-center font-black">残チケット</div><div className={`text-6xl font-black text-center ${ (cust.tickets || 0) <= 0 ? 'text-red-500' : 'text-[#b4927b]'}`}>{cust.tickets || 0} 枚</div></div>
                <div className="flex space-x-4 text-center font-black text-center"><button onClick={() => adjustTickets(cust.id, -1)} className="p-7 bg-slate-100 rounded-[32px] active:scale-90 transition-all shadow-md text-center"><Minus size={48} className="text-center"/></button><button onClick={() => adjustTickets(cust.id, 1)} className="p-7 bg-[#fcf8f5] text-[#b4927b] rounded-[32px] active:scale-90 transition-all border-4 border-white shadow-md text-center text-center"><Plus size={48} className="text-center"/></button><button onClick={() => adjustTickets(cust.id, 4)} className="px-12 py-7 bg-[#b4927b] text-white rounded-[32px] text-2xl shadow-2xl active:scale-95 transition-all text-center text-center">＋4枚</button></div></div>
              </div>
            ))}
            {customers.length === 0 && <div className="text-center py-32 text-slate-400 font-black text-2xl text-center">登録された生徒さんはまだいません</div>}
          </div>
        )}

        {!isAdminMode && step === 4 && (
          <div className="animate-in slide-in-from-right pt-10 text-left text-left text-left">
            <h2 className="text-2xl font-black mb-10 border-l-8 border-[#b4927b] pl-6 text-slate-900 text-left">ご予約内容の確認</h2>
            <div className="bg-white rounded-[72px] p-16 border-2 shadow-2xl mb-16 text-left font-black text-left text-left text-left">
              <div className="space-y-12 mb-12 text-left text-left text-left">
                <div className="flex justify-between border-b-2 pb-8 text-slate-500 text-xl text-left"><span className="text-left font-black">メニュー</span><span className="text-3xl text-[#b4927b] font-black text-right">{selectedMenu ? selectedMenu.name : '少人数制グループ'}</span></div>
                <div className="flex flex-col border-b-2 pb-8 text-slate-500 text-xl text-left"><span className="mb-4 text-left font-black">日時</span><span className="text-3xl text-slate-900 leading-relaxed font-black text-left">{selectedDate.toLocaleDateString('ja-JP', { month: 'long', day: 'numeric' })} ({selectedDate.toLocaleDateString('ja-JP', { weekday: 'short' })}) <br className="text-left"/> {targetTime} 〜 {calculateEndTime(selectedDate, targetTime, selectedMenu ? selectedMenu.duration : GROUP_MIN)}</span></div>
              </div>
              <div className="bg-[#fcf8f5] rounded-[48px] p-12 shadow-inner border-2 border-white text-center">
                ...
                <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-white border-4 border-white rounded-[32px] py-8 px-10 text-4xl text-center focus:border-[#b4927b] outline-none transition-all shadow-xl font-black text-center" placeholder="例：山田 太郎" />
              </div>
            </div>
            <button onClick={handleSubmit} disabled={loading || !customerName.trim()} className="w-full bg-[#b4927b] text-white font-black py-10 rounded-full shadow-2xl active:scale-95 disabled:bg-gray-200 text-4xl transition-all shadow-[#b4927b]/40 text-center text-center">{loading ? "送信中..." : "上記の内容で予約確定"}</button>
          </div>
        )}
        ...
        {!isAdminMode && step === 5 && (
          <div className="text-center py-40 animate-in zoom-in px-4 text-center">
            ...
            <p className="text-3xl text-slate-600 font-bold mb-24 leading-relaxed text-center text-center">ご予約ありがとうございます。<br className="text-center"/>当日、お会いできるのを<br className="text-center"/>楽しみにしております。</p>
            <button onClick={() => setStep(1)} className="w-full py-10 bg-[#b4927b] text-white font-black rounded-full shadow-2xl text-4xl active:scale-95 transition-all text-center">トップページへ戻る</button>
          </div>
        )}
      </main>

      {/* Footer Nav */}
      <footer className="fixed bottom-0 left-0 right-0 bg-white/98 border-t-4 border-gray-50 px-10 py-10 flex justify-around items-center z-30 shadow-[0_-20px_60px_rgba(0,0,0,0.08)] backdrop-blur-2xl text-center text-center">
        <div className={`flex flex-col items-center cursor-pointer transition-all text-center text-center ${!isAdminMode ? 'text-[#b4927b] scale-125' : 'text-slate-300'}`} onClick={() => {setStep(1); setIsAdminMode(false);}}>
          <CalendarIcon size={56} strokeWidth={2.5} className="text-center"/><span className="text-[16px] font-black mt-3 uppercase tracking-widest text-center">予約</span>
        </div>
        <div className={`flex flex-col items-center cursor-pointer transition-all text-center text-center ${isAdminMode ? 'text-[#b4927b] scale-125' : 'text-slate-300'}`} onClick={() => { if(!isAdminMode) setStep(6); else setAdminTab('ledger'); }}>
          <CalendarCheck size={56} strokeWidth={2.5} className="text-center"/><span className="text-[16px] font-black mt-3 uppercase tracking-widest text-center">台帳</span>
        </div>
        <div className="flex flex-col items-center text-gray-300 text-center"><Info size={56} strokeWidth={2.5} className="text-center"/><span className="text-[16px] font-black mt-3 uppercase tracking-widest text-center">情報</span></div>
      </footer>
    </div>
  );
}
